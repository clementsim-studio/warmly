...
**Decision**: GA4 (`src/lib/analytics.js`) is one of Warmly's two analytics integrations — the other is Vercel Web Analytics (cookieless, no consent needed, added the same day; see CHANGELOG). GA4's `gtag.js` loads only when `import.meta.env.PROD` **and** `VITE_GA4_MEASUREMENT_ID` is set — so it's off in `vite dev`, off in previews, and switched on purely by setting the Vercel env var. The script tag is injected `async` from JS rather than hard-coded in `index.html`, so it never blocks first paint and the dev/prod gate lives in one place.
...
**Consent — Consent Mode v2 denied by default, cookieless, and no banner (deliberately deferred)**: `initAnalytics()` sets Consent Mode v2 `default` to `denied` for `ad_storage`, `ad_user_data`, `ad_personalization`, and `analytics_storage` before `config`. GA4 then runs cookieless — it sends pings, sets no `_ga` cookie, and Google models the gaps.

A lightweight consent banner **was** designed as the follow-up — a small first-visit prompt that would call `initAnalytics()` (or flip `analytics_storage` to `granted`) only on opt-in. It is deliberately **not built**, and the reason is product/UX, not effort or scope: Warmly is a new, warm, personal app, and a cookie-consent popup on first visit signals "we're tracking you" before the visitor has seen what the app is or done anything with it. That first impression costs more than the data is currently worth — and right now it's worth almost nothing measurable. With `analytics_storage` denied and nothing ever granting it, GA4 shows **zero in Realtime** and won't populate standard reports until behavioral-modeling thresholds are met (~1,000 denied events/day + ~1,000 granted users/day over 7+ days — far beyond current traffic; confirmed in-session). So skipping the banner sacrifices no analytics value we can actually see today; we are not meaningfully better off with it than without it at this stage.

**Not flying blind meanwhile**: Vercel Web Analytics (cookieless, needs no consent mechanism) stays active regardless of this decision — page views, referrers, geo, device breakdowns. What the deferred banner costs is GA4's richer event-level detail: the `card_created` / `card_shared` / `card_signed` / `feedback_submitted` funnel and the `occasion` / `format` / `rating` dimensions, which Vercel Analytics doesn't capture. That gap is the accepted trade until this is revisited.

**Trigger to revisit** — build the banner (or otherwise move `analytics_storage` to `granted` on consent) if **any** of:
- (a) Warmly gets enough traffic that the missing GA4 event-level data actually matters for a decision — the funnel / occasion / format breakdowns would change what we do, and Vercel Analytics' page-level view isn't enough;
- (b) EU/UK legal exposure becomes a real concern at scale — a meaningful volume of EU/UK visitors, or the project takes on something that raises the compliance stakes;
- (c) the product matures enough that a brief, low-key consent ask no longer reads as jarring on a first visit — at which point the first-impression objection above no longer applies.
...
## Drawings are communal like stickers; the move grip shows on every selected object (D-055)

**Decision**: Freehand drawings on the Content face can be moved, resized, rotated and removed by **anyone**, matching stickers — `type === 'draw'` is added everywhere `type === 'sticker'` is already checked (`CardScreen.jsx`: the pinch `canManip`, the `startMove` ownership guard, and `grabbable`). Text and photos stay owner-only. Separately, `showMoveGrip` is now `grabbable && selected` (was also gated on `isThisEditing`), so the grip appears on every selected object.

**Why**: The ownership rule protects *authored* contributions — a message with your name on it, a photo you chose. A doodle carries no signature, exactly like a sticker, and the fix-up case is identical: someone draws across another person's note and anyone present should be able to nudge it. Cover drawings were already movable only because cover objects carry `communal: true`; content drawings were the anomaly. No data-model change — content drawings keep `communal: false` and are treated as communal purely by type, the same way content stickers always have been. The move grip was gated on an implementation detail (on an editing note, body-drag is reserved for text selection, so a separate handle is needed there) — but a *missing* grip made stickers/drawings/photos look less manipulable than a note. Showing it everywhere makes the selection UI mean one consistent thing; body-drag still works underneath.

## A popover's own entrance animation can override its positioning (D-056)

**Decision**: A popover centred with `transform: translateX(-50%)` must animate with `@keyframes fadeUpCx` (`translate(-50%,10px)` → `translate(-50%,0)`), which carries the centring offset in every frame — not the generic `fadeUp`. Popovers also each anchor to their own control via a dedicated `position: relative` wrapper, never to a shared action group's edge. Applied to the mobile ⋯ menu (own wrapper around the ⋯ button; `left:50%` + `translateX(-50%)`; `fadeUpCx`) and the mobile Signatures popover (moved into the same ⋯ wrapper, since on mobile it's opened from inside the ⋯ menu). The desktop Signatures popover already had its own relative wrapper.

**Why**: An animated `transform` wins over the inline `transform` for the animation's whole duration — `fadeUp` sets `translateY(...)`, discarding the inline `translateX(-50%)` for 200ms, so the panel opened off-centre and snapped into place. Nothing was wrong with the positioning; the animation was quietly replacing it (same class of bug as D-042 — when something is briefly right then wrong, look for what runs *after* it). Anchoring to a shared container is fragile: both the ⋯ menu and the Signatures popover used `left:0`/`right:0` against a multi-button group, which only looked correct while the opening control sat at that edge — reordering the bar detached them. A per-control wrapper makes the anchor structural rather than coincidental. (Hard-won constraint #12/#13.)

**Note on the mobile top-bar order**: the fifth handoff also specified "mobile order ⋯ · Share · Download via flex `order`, not DOM order." This codebase renders the mobile top bar from its own `mobileView ? … : …` branch whose DOM order already is ⋯ · Share · Download, so no flex `order` was needed — the reference uses `order` only because it shares one DOM tree between desktop and mobile.

## `card_objects` / `signers` use `REPLICA IDENTITY FULL` so realtime DELETEs pass the row filter

**Decision**: `alter table card_objects replica identity full;` and the same for `signers` (migration `0010_replica_identity_full.sql`).

**Why**: `subscribeToCard` (`src/lib/cardData.js`) subscribes to `postgres_changes` with a `card_id=eq.<cardId>` row filter so each browser only hears about its own card. Supabase Realtime applies that filter to the change payload. INSERT/UPDATE payloads carry every column (`payload.new`), so the filter matches. A DELETE payload carries only the columns in the table's `REPLICA IDENTITY`, and the Postgres default is the primary key alone — so a DELETE here was just `{ id }`, had no `card_id`, failed the filter, and Realtime dropped it. Result: adds and edits synced live between viewers, deletes did not (the other browser only caught up on refresh). `REPLICA IDENTITY FULL` puts the whole old row into the replication stream on UPDATE/DELETE, so the DELETE payload now has `card_id` and the filter matches. The client's DELETE handler (`setObjects(prev => prev.filter(o => o.id !== oldRow.id))`) was already correct — it just never received the event.

**Cost**: UPDATE/DELETE WAL entries on these two tables carry the full old row instead of just the id. `card_objects` rows are small (a short note or one SVG path), so this is negligible here; no effect on reads, inserts, RLS, or app logic. The alternative — dropping the filter and matching client-side — would fan every card's deletes out to every subscriber, which scales badly and leaks activity timing; a per-table replica-identity change is the contained fix.
