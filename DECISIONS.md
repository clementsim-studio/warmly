...
## Zoom-to-write centres the current viewport, not the re-edited note's own position

**Decision**: `zoomToWriteIfNeeded()` zooms in around the centre of whatever is *currently on screen* (reusing `setZoomAt`'s existing "keep this screen point anchored through the zoom" behavior), for both starting a new note and re-editing an existing one. It does not scroll to bring an off-screen note into frame before zooming.

**Why**: New notes are already placed at the view centre (D-034), so this is exactly correct for that case with no extra work. For re-editing, a fully faithful implementation would first compute the tapped note's own on-canvas centre and scroll-then-zoom to frame it precisely — but that requires reasoning about the scroll container's coordinate space (padding, the surface box's `margin:auto` centring) that's much safer to get right with a real browser to test against than to derive from first principles alone. Since re-editing almost always targets a note already close to the current view (you tapped it because you could see it), zooming in place is a reasonable approximation. Documented here so this reads as a deliberate, known trade-off rather than an oversight if it's ever revisited.
